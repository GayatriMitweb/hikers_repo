...
public function exc_refund_save()
{
	...
	$sq_max = mysql_fetch_assoc(mysql_query("select max(refund_id) as max from exc_refund_master"));
	$refund_id = $sq_max['max'] + 1;

	$sq_payment = mysql_query("insert into exc_refund_master (refund_id, exc_id, financial_year_id, refund_date, refund_amount, refund_mode, bank_name, transaction_id, bank_id, clearance_status, created_at) values ('$refund_id', '$exc_id', '$financial_year_id', '$refund_date', '$refund_amount', '$refund_mode', '$bank_name', '$transaction_id', '$bank_id', '$clearance_status', '$created_at') ");
	...
	if($refund_mode == 'Credit Note'){
		...
		$sq_payment = mysql_query("insert into credit_note_master (id, financial_year_id, module_name, module_entry_id, customer_id, payment_amount,refund_id,created_at,branch_admin_id) values ('$id', '$financial_year_id', 'Excursion Booking', '$exc_id', '$customer_id','$refund_amount','$refund_id','$refund_date','$branch_admin_id') ");
	}
	...
	if(!$sq_payment){
		...
		for($i=0; $i<sizeof($entry_id_arr); $i++){
			...
			$sq_max = mysql_fetch_assoc(mysql_query("select max(id) as max from exc_refund_entries"));
			$id= $sq_max['max'] + 1;;

			$sq_entry = mysql_query("insert into exc_refund_entries(id, refund_id, entry_id) values ('$id', '$refund_id', '$entry_id_arr[$i]')");
			if(!$sq_entry){
				$GLOBALS['flag'] = false;
...
public function finance_save($refund_id)
{
	...
	$sq_exc = mysql_fetch_assoc(mysql_query("select * from excursion_master_entries where exc_id='$exc_id'"));
	$sq_t = mysql_fetch_assoc(mysql_query("select excursion_name from excursion_master_tariff where entry_id='$sq_exc[exc_name]'"));
...
global $app_email_id, $app_name, $app_contact_no, $admin_logo_url, $app_website,$encrypt_decrypt,$secret_key,$currency_logo;
  ...
  $yr = explode("-", $date);
  $year =$yr[0];
  $cust_email = mysql_fetch_assoc(mysql_query("select * from customer_master where customer_id='$sq_exc_info[customer_id]'"));
  ...
  <tr>
      <table width="85%" cellspacing="0" cellpadding="5" style="color: #888888;border: 1px solid #888888;margin: 0px auto;margin-top:20px; min-width: 100%;" role="presentation">
          <tr><td style="text-align:left;border: 1px solid #888888;">Service Type</td>   <td style="text-align:left;border: 1px solid #888888;">Excursion Booking</td></tr>
...
// }
   ...
   $subject = 'Excursion Cancellation Refund( '.get_exc_booking_id($sq_sq_train_info['exc_id'],$year).' )';
  global $model;
 
  $model->app_email_send('43',$cust_email['first_name'],$email_id, $content,$subject);
 }
