...
public function finance_save($refund_id)
{
	...
	$particular = 'Payment through '.$refund_mode.' '.get_train_ticket_booking_id($train_ticket_id,$yr1).' for tkt of '.$cust_name.' * '.$pax.' traveling for '.$sector.' against ticket no '.$ticket_number.' by '.$train_no.'/'.$class;	
...
global $app_email_id, $app_name, $app_contact_no, $admin_logo_url, $app_website,$encrypt_decrypt,$secret_key,$currency_logo;
  ...
  $yr = explode("-", $date);
  $year =$yr[0];
  $cust_email = mysql_fetch_assoc(mysql_query("select * from customer_master where customer_id='$sq_train_ticket_info[customer_id]'"));
  $email_id = $encrypt_decrypt->fnDecrypt($cust_email['email_id'], $secret_key);
  $sq_payment = mysql_fetch_assoc(mysql_query("select sum(payment_amount) as sum_pay from train_ticket_payment_master where train_ticket_id='$sq_train_ticket_info[train_ticket_id]' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
  $sq_refund_amount = mysql_fetch_assoc(mysql_query("select sum(refund_amount) as sum from train_ticket_refund_master where train_ticket_id='$sq_train_ticket_info[train_ticket_id]' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
...
$subject = 'Train Ticket Cancellation Refund ('.get_train_ticket_booking_id($sq_sq_train_info['train_ticket_id'],$year).' )';
  ...
  $model->app_email_send('35',$cust_email['first_name'],$email_id, $content,$subject);
 }
}
?>