...
public function booking_save(){
  ...
  $bsmValues = json_encode($bsmValues);
	$sq_booking = mysql_query("INSERT INTO hotel_booking_master(booking_id, quotation_id,customer_id, branch_admin_id,financial_year_id,pass_name, adults, childrens, infants, sub_total, service_charge, service_tax_subtotal,markup,markup_tax, discount, tds, total_fee, unique_timestamp, due_date , created_at, emp_id,reflections,bsm_values,roundoff) VALUES ('$booking_id', '$quotation_id','$customer_id', '$branch_admin_id','$financial_year_id','$pass_name', '$adults', '$childrens', '$infants', '$sub_total', '$service_charge', '$service_tax_subtotal', '$markup','$service_tax_markup','$discount', '$tds', '$total_fee', '$unique_timestamp', '$due_date', '$booking_date', '$emp_id','$reflections','$bsmValues','$roundoff')");
	  if(!$sq_booking){
        rollback_t();
      ...
      for($i=0; $i<sizeof($city_id_arr); $i++){
	    ...
	    $payment_id = $sq_max['max']+1;
	  	$sq_payment = mysql_query("insert into hotel_booking_payment(payment_id, booking_id, financial_year_id, branch_admin_id, payment_date, payment_mode, payment_amount, bank_name, transaction_id, bank_id, clearance_status, created_at,credit_charges,credit_card_details) values ('$payment_id', '$booking_id', '$financial_year_id', '$branch_admin_id', '$payment_date', '$payment_mode', '$payment_amount', '$bank_name', '$transaction_id', '$bank_id', '$clearance_status', '$created_at','$credit_charges','$credit_card_details')");
	    if(!$sq_payment){
	  	  $GLOBALS['flag'] = false;
      ...
      $particular = $this->get_particular($customer_id,$pax,$no_of_nights_arr[0],$check_in_arr[0],$category_arr[0],$hotel_id_arr[0]);
    	//Finance save
...
function get_particular($customer_id,$pax,$nights,$check_date,$room_type,$hotel_id){
  ...
  $sq_ht = mysql_fetch_assoc(mysql_query("select hotel_name from hotel_master where hotel_id='$hotel_id'"));
...
public function booking_update()
{
  ...
  $reflections = json_encode($reflections);
  $bsmValues = json_encode($bsmValues);
	$sq_booking = mysql_query("UPDATE hotel_booking_master SET customer_id='$customer_id',pass_name='$pass_name', adults='$adults', childrens='$childrens', infants='$infants', sub_total='$sub_total', service_charge='$service_charge',service_tax_subtotal='$service_tax_subtotal',markup='$markup', discount='$discount', tds='$tds', total_fee='$total_fee', due_date='$due_date1',created_at='$booking_date1',reflections='$reflections',markup_tax='$service_tax_markup',bsm_values='$bsmValues',roundoff='$roundoff'  WHERE booking_id='$booking_id'");
	if($sq_booking){
