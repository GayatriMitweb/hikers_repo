...
public function hotel_refund_save()
{
	...
	$sq_max = mysql_fetch_assoc(mysql_query("select max(refund_id) as max from hotel_booking_refund_master"));
	$refund_id = $sq_max['max'] + 1;

	$sq_payment = mysql_query("insert into hotel_booking_refund_master (refund_id, booking_id, financial_year_id, refund_date, refund_amount, refund_mode, bank_name, transaction_id, bank_id, clearance_status, created_at) values ('$refund_id', '$booking_id', '$financial_year_id', '$refund_date', '$refund_amount', '$refund_mode', '$bank_name', '$transaction_id', '$bank_id', '$clearance_status', '$created_at') ");
	...
	if(!$sq_payment){
		...
		for($i=0; $i<sizeof($entry_id_arr); $i++){
			...
			$sq_max = mysql_fetch_assoc(mysql_query("select max(id) as max from hotel_booking_refund_entries"));
			$id= $sq_max['max'] + 1;;

			$sq_entry = mysql_query("insert into hotel_booking_refund_entries(id, refund_id, entry_id) values ('$id', '$refund_id', '$entry_id_arr[$i]')");
			if(!$sq_entry){
				$GLOBALS['flag'] = false;
...
public function finance_save($refund_id){
	...
	$particular = 'Payment through '.$refund_mode.' for '.get_hotel_booking_id($booking_id,$yr1).' for '.$sq_hte['no_of_nights'].' Night(s) stay from '.get_date_user($sq_hte['check_in']).' in '.$sq_hte['category'].' Room at '.$hotel_name .' * '.($sq_hotel_info['adults']+$sq_hotel_info['childrens']).' '.$cust_name;
...
public function refund_mail_send($booking_id,$refund_amount,$refund_date,$refund_mode,$transaction_id){
  ...
  $sq_sq_train_info = mysql_fetch_assoc(mysql_query("select * from hotel_booking_master where booking_id='$booking_id'"));
