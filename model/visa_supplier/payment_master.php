...
public function payment_save()
{
	...
	$sq_max = mysql_fetch_assoc(mysql_query("select max(id) as max from visa_supplier_payment"));
	$advance_id = $sq_max['max'] + 1;

	$sq_income = mysql_query("insert into visa_supplier_payment (id, supplier_id, financial_year_id, payment_amount, payment_date, payment_mode, bank_name, transaction_id, bank_id, clearance_status, created_at) values ('$advance_id', '$sup_id', '$financial_year_id', '$payment_amount', '$payment_date', '$payment_mode', '$bank_name', '$transaction_id', '$bank_id', '$clearance_status', '$created_at')");
	if($sq_income)
	{
		$sq_vendor = mysql_fetch_assoc(mysql_query("select * from visa_vendor where vendor_id='$sup_id'"));
		$vendor_name = $sq_vendor['vendor_name'];
		$vendor_name = addslashes($vendor_name);
		$sq_vendor_count = mysql_num_rows(mysql_query("select * from ledger_master where customer_id='$sup_id' and user_type='Visa Vendor' and group_sub_id='23'"));
		if($sq_vendor_count != '0' ){
			$sq_vendor_ledger = mysql_fetch_assoc(mysql_query("select * from ledger_master where customer_id='$sup_id' and user_type='Visa Vendor' and group_sub_id='23'"));
		    ...
		    $sq_ledger = mysql_query("insert into ledger_master (ledger_id, ledger_name, alias, group_sub_id, balance, dr_cr,customer_id,user_type) values ('$ledger_id', '$ledger_name', '', '23', '0','Dr','$sup_id','Visa Vendor')");
		}
		...
		global $model;
		$sq_supplier = mysql_fetch_assoc(mysql_query("select * from visa_vendor where vendor_id='$sup_id'"));
   	    $model->while_topup_mail_send($payment_amount, $sq_supplier['vendor_name'],'visa');
...
public function finance_save($payment_id,$ledger_id,$vendor_name)
{
	...
	//////Payment Amount///////
    $module_name = 'Visa Vendor';
    $module_entry_id = $payment_id;
    $transaction_id = $transaction_id1;
    $payment_amount = $payment_amount1;
    $payment_date = $payment_date;
    $payment_particular = get_visa_supplier_particular($vendor_name);
    $ledger_particular = '';
    $gl_id = $pay_gl;
    $payment_side = "Credit";
	$clearance_status = ($payment_mode=="Cheque") ? "Pending" : "";
...
public function bank_cash_book_save($advance_id,$vendor_name)
{
	...
	$module_name = "Visa Supplier Payment";
	...
	$transaction_id = $transaction_id1;
	$bank_id = $bank_id;
	$particular = get_visa_supplier_particular($vendor_name);
	$clearance_status = ($payment_mode=="Cheque") ? "Pending" : "";
	$payment_side = "Credit";
...
public function payment_update()
{
	...
	$sq_payment_info = mysql_fetch_assoc(mysql_query("select * from visa_supplier_payment where id='$pay_id'"));
