...
while($row_booking = mysql_fetch_assoc($sq_booking)){
    $sq_regs = mysql_fetch_assoc(mysql_query("select register_id,email_id,cp_first_name,cp_last_name from b2b_registration where customer_id='$row_booking[customer_id]' and approval_status='Approved'"));
    echo '$email_id : '.$sq_regs['email_id'];
    $sq_count = mysql_num_rows(mysql_query("select * from b2b_creditlimit_master where register_id='$sq_regs[register_id]' and approval_status='Approved'"));
    if($sq_count != 0){
        $hotel_total = 0;
