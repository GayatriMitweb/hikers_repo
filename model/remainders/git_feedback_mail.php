...
if($sq_tour_group>0){
	...
	while($row_tour = mysql_fetch_assoc($sq_tour_group1)){
		...
		while ($row_tour1 = mysql_fetch_assoc($sq_tour)){
			...
			$sq_bookings = mysql_query("select * from tourwise_traveler_details where tour_id='$tour_id' and tour_group_id='$tour_group_id'");
			while($row_bookings = mysql_fetch_assoc($sq_bookings)){
				...
				$sq_personal_info =  mysql_query("select * from traveler_personal_info where tourwise_traveler_id='$tourwise_traveler_id'");
				while($row_per = mysql_fetch_assoc($sq_personal_info)){
					...
					$sq_count = mysql_num_rows(mysql_query("SELECT * from  remainder_status where remainder_name = 'git_feedback_remainder' and date='$today' and status='Done'"));
					if($sq_count==0)
					{
		
					feedback_email_send($email_id,$tourwise_traveler_id1,$tour_name,$journey_date,$username,$password,$row_cust['first_name'],$customer_id);
					}
					if($mobile_no!=""){
...
$max=$value['max']+1;
$sq_check_status=mysql_query("INSERT INTO `remainder_status`(`id`, `remainder_name`, `date`, `status`) VALUES ('$max','git_feedback_remainder','$today','Done')");
