...
while($row_query = mysql_fetch_assoc($sq_query))
{
    ...
    $sq_supplier1 = mysql_fetch_assoc(mysql_query("select sum(payment_amount) as total_amount1 from vendor_advance_master where vendor_type_id='$row_query[vendor_id]' and vendor_type='Ticket Vendor' and (clearance_status!='Pending' and clearance_status!='Cancelled')"));
    ...
    if($sq_supplier['total_amount'] != 0 || $sq_supplier1['total_amount1'] != 0) {
        ...
        if($balance_amount<'50000'){
            ...
            $sq_count = mysql_num_rows(mysql_query("SELECT * from  remainder_status where remainder_name = 'topup_recharge_remainder' and date='$due_date' and status='Done'"));
            ...
            if($sq_count==0){
                ...
                $max = $value['max']+1;
                $sq_check_status = mysql_query("INSERT INTO `remainder_status`(`id`, `remainder_name`, `date`, `status`) VALUES ('$max','topup_recharge_remainder','$due_date','Done')");
                $model->topup_remainder_mail($balance_amount, $row_query['vendor_name']);
            }
        }
    }
