...
function enquiry_master_save(){
  ...
  if($by == 'cust'){
    $cur_date = date('Y-m-d');
    $sq_fin = mysql_fetch_assoc(mysql_query("select financial_year_id from financial_year where `from_date` <= '$cur_date' and `to_date` >= '$cur_date' order by `financial_year_id` desc"));
