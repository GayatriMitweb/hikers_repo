...
while($row_query = mysql_fetch_assoc($sq_query))
{
  ...
  while($row_finance = mysql_fetch_assoc($sq_finance)){
    ...
    $customer_name = get_customer_name($row_finance['module_name'],$row_finance['module_entry_id']);
        ...
        ->setCellValue('C'.$row_count, $row_finance['module_name'])
...
header('Content-Disposition: attachment;filename="Sales_Register('.date('d-m-Y H:i:s').').xls"');
