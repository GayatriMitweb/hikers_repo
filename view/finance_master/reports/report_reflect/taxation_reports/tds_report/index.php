...
<table id="tds_rec" class="table table-hover" style="margin: 20px 0 !important;">         
</table>
</div></div></div></div>
...
function report_reflect()
{
	...
	$.post('report_reflect/taxation_reports/tds_report/report_reflect.php',{ from_date : from_date, to_date : to_date,branch_status : branch_status, role : role,branch_admin_id : branch_admin_id  }, function(data){
		// console.log(data);
		pagination_load(data, column, true, false, 20, 'tds_rec');
	});
}
report_reflect();
function excel_report()
{
	...
	window.location = 'report_reflect/taxation_reports/tds_report/excel_report.php?from_date='+from_date+'&to_date='+to_date+'&branch_status='+branch_status+'&branch_admin_id='+branch_admin_id+'&role='+role;
}
</script>
