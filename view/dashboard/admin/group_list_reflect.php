...
$query = mysql_fetch_assoc(mysql_query("select max(id) as booking_id from tourwise_traveler_details where financial_year_id='$financial_year_id'"));
          ...
          <div class="table-responsive no-marg-sm">
            <table class="table table-hover" style="margin: 0 !important;border: 0;padding: 0 !important;">
              ...
              while($row_entry1 = mysql_fetch_assoc($sq_entry)){
                ...
                $actual_tour_expense = $row_entry['net_total'] ;
                //paid
                $query = mysql_fetch_assoc(mysql_query("SELECT sum(amount) as sum,sum(credit_charges) as sumc from payment_master where tourwise_traveler_id='$booking_id' and clearance_status != 'Pending' and clearance_status != 'Cancelled'"));
                $paid_amount = $query['sum']+$query['sumc'];
                $paid_amt = ($paid_amount == '')?'0':$paid_amount;

                if($row_entry['tour_group_status'] == 'Cancel'){
                //Group Tour cancel
                $cancel_tour_count2=mysql_num_rows(mysql_query("SELECT * from refund_tour_estimate where tourwise_traveler_id='$booking_id'"));
                if($cancel_tour_count2 >= '1'){
                  $cancel_tour=mysql_fetch_assoc(mysql_query("SELECT * from refund_tour_estimate where tourwise_traveler_id='$booking_id'"));
                ...
                $cancel_esti_count1=mysql_num_rows(mysql_query("SELECT * from refund_traveler_estimate where tourwise_traveler_id='$booking_id'"));
                if($cancel_esti_count1 >= '1'){
                  $cancel_esti1=mysql_fetch_assoc(mysql_query("SELECT * from refund_traveler_estimate where tourwise_traveler_id='$booking_id'"));
              ...
              ?>
                <tr class="<?= $bg ?>">
                    ...
                    <td>
                      <button class="btn btn-info btn-sm" title="ID Proof" onclick="display_group_id_proof('<?php echo $row_entry1['id_proof_url']; ?>')"><i class="fa fa-id-card-o"></i></button>
                    </td>
                </tr>
                  <?php } ?>
              </tbody>
