			...
			if($sq_plane_count==0){
				...
				while($row_q_hotel = mysql_fetch_assoc($sq_q_hotel)){
                    ...
                    <tr>
                        ...
                        <?php
                        $query = mysql_query("select * from hotel_master where city_id = ".$row_q_hotel['city_id']);
                        while($row = mysql_fetch_assoc($query)){
                            $selected = "";
                            if($row_q_hotel['hotel_id'] == $row['hotel_id']){
                                $selected = "selected";
                            }
                        ?>
                                <option value="<?= $row['hotel_id'] ?>" <?= $selected ?> ><?= $row['hotel_name'] ?></option>
                        <?php
