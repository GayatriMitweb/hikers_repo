...
if($from_date!="" && $to_date!=""){ 
    ...
    $sq_emp = mysql_fetch_assoc(mysql_query("select * from emp_master where emp_id='$sq_eq_emp[value_occurrence]'"));

    $query1 = "SELECT `assigned_emp_id`, COUNT(`assigned_emp_id`) AS `value_occurrence` FROM `enquiry_master` where enquiry_id in (select enquiry_id from enquiry_master_entries where followup_status='Converted'  and status!='False') and enquiry_date between '$from_date1' and '$to_date1' and status!='Disabled' GROUP BY  `assigned_emp_id` ORDER BY `value_occurrence` DESC LIMIT 1";
    $sq_eq_emp1 = mysql_fetch_assoc(mysql_query($query1));
    $sq_emp1 = mysql_fetch_assoc(mysql_query("select * from emp_master where emp_id='$sq_eq_emp1[value_occurrence]'"));
}
?>
