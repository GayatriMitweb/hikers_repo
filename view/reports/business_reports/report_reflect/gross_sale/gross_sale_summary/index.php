...
<!-- Group Booking -->
<?php
$sq_group_count = mysql_num_rows(mysql_query("select * from tourwise_traveler_details where financial_year_id='$financial_year_id' and tour_group_status != 'Cancel'"));
$sq_group_pre_count = mysql_num_rows(mysql_query("select * from tourwise_traveler_details where financial_year_id='$pre_financial_year_id' and tour_group_status != 'Cancel'"));

$sq_group = mysql_fetch_assoc(mysql_query("select sum(net_total) as net_total from tourwise_traveler_details where financial_year_id='$financial_year_id' and tour_group_status != 'Cancel'"));
$sq_group_pre = mysql_fetch_assoc(mysql_query("select sum(net_total) as net_total from tourwise_traveler_details where financial_year_id='$pre_financial_year_id' and tour_group_status != 'Cancel'"));
$current_group_sale = $sq_group['net_total'];
$previous_group_sale = $sq_group_pre['net_total'];

$sq_group_month = mysql_fetch_assoc(mysql_query("SELECT sum(net_total) as net_total FROM tourwise_traveler_details WHERE MONTH(form_date) = $month AND financial_year_id='$financial_year_id' and tour_group_status != 'Cancel'"));
$sq_group_month_pre = mysql_fetch_assoc(mysql_query("SELECT sum(net_total) as net_total FROM  tourwise_traveler_details WHERE MONTH(form_date) = $month AND financial_year_id='$pre_financial_year_id' and tour_group_status != 'Cancel'"));
...
$sq_package = mysql_fetch_assoc(mysql_query("select sum(net_total) as net_total from package_tour_booking_master where financial_year_id='$financial_year_id'"));
$sq_package_pre = mysql_fetch_assoc(mysql_query("select sum(net_total) as net_total from package_tour_booking_master where financial_year_id='$pre_financial_year_id'"));

$current_package_sale = $sq_package['net_total'];
$previous_package_sale = $sq_package_pre['net_total'];

$sq_package_month = mysql_fetch_assoc(mysql_query("SELECT sum(net_total) as net_total FROM package_tour_booking_master WHERE MONTH(booking_date) = $month AND financial_year_id='$financial_year_id'"));
$sq_package_month_pre = mysql_fetch_assoc(mysql_query("SELECT sum(net_total) as net_total FROM package_tour_booking_master WHERE MONTH(booking_date) = $month AND financial_year_id='$pre_financial_year_id'"));
...
$previous_package_sale_m = $sq_package_month_pre['net_total'];
 ...
 <!-- Hotel Booking -->
<?php
$sq_hotel_count = mysql_num_rows(mysql_query("select * from hotel_booking_master where financial_year_id='$financial_year_id' and booking_id in(SELECT booking_id FROM `hotel_booking_entries` WHERE `status`!='Cancel' group by booking_id)"));
$sq_hotel_pre_count = mysql_num_rows(mysql_query("select * from hotel_booking_master where financial_year_id='$pre_financial_year_id' and booking_id in(SELECT booking_id FROM `hotel_booking_entries` WHERE `status`!='Cancel' group by booking_id)"));

$sq_hotel = mysql_fetch_assoc(mysql_query("select sum(total_fee) as hotel_total from hotel_booking_master where financial_year_id='$financial_year_id' and booking_id in(SELECT booking_id FROM `hotel_booking_entries` WHERE `status`!='Cancel' group by booking_id)"));
$sq_hotel_pre = mysql_fetch_assoc(mysql_query("select sum(total_fee) as hotel_total from hotel_booking_master where financial_year_id='$pre_financial_year_id' and booking_id in(SELECT booking_id FROM `hotel_booking_entries` WHERE `status`!='Cancel' group by booking_id)"));
...
$current_month_hotel = $sq_hotel_month['hotel_total'];
    ...
    <?php }
// Bus Booking
$sq_bus_count = mysql_num_rows(mysql_query("select * from bus_booking_master where financial_year_id='$financial_year_id' and booking_id in(SELECT booking_id FROM `bus_booking_entries` WHERE `status`!='Cancel' group by booking_id)"));
$sq_bus_pre_count = mysql_num_rows(mysql_query("select * from bus_booking_master where financial_year_id='$pre_financial_year_id' and booking_id in(SELECT booking_id FROM `bus_booking_entries` WHERE `status`!='Cancel' group by booking_id)"));

$sq_bus = mysql_fetch_assoc(mysql_query("select sum(net_total) as bus_total from bus_booking_master where financial_year_id='$financial_year_id' and booking_id in(SELECT booking_id FROM `bus_booking_entries` WHERE `status`!='Cancel' group by booking_id)"));
$sq_bus_pre = mysql_fetch_assoc(mysql_query("select sum(net_total) as bus_total from bus_booking_master where financial_year_id='$pre_financial_year_id' and booking_id in(SELECT booking_id FROM `bus_booking_entries` WHERE `status`!='Cancel' group by booking_id)"));
...
<?php } 
// Flight Booking
$sq_flight_count = mysql_num_rows(mysql_query("select * from ticket_master where financial_year_id='$financial_year_id' and ticket_id in(SELECT ticket_id FROM `ticket_master_entries` WHERE `status`!='Cancel' group by ticket_id)"));
$sq_flight_pre_count = mysql_num_rows(mysql_query("select * from ticket_master where financial_year_id='$pre_financial_year_id' and ticket_id in(SELECT ticket_id FROM `ticket_master_entries` WHERE `status`!='Cancel' group by ticket_id)"));

$sq_flight = mysql_fetch_assoc(mysql_query("select sum(ticket_total_cost) as flight_total from ticket_master where financial_year_id='$financial_year_id' and ticket_id in(SELECT ticket_id FROM `ticket_master_entries` WHERE `status`!='Cancel' group by ticket_id)"));
$sq_flight_pre = mysql_fetch_assoc(mysql_query("select sum(ticket_total_cost) as flight_total from ticket_master where financial_year_id='$pre_financial_year_id' and ticket_id in(SELECT ticket_id FROM `ticket_master_entries` WHERE `status`!='Cancel' group by ticket_id)"));
...
<?php } 
// <!-- Train Booking -->
$sq_train_count = mysql_num_rows(mysql_query("select * from  train_ticket_master where financial_year_id='$financial_year_id' and train_ticket_id in(SELECT train_ticket_id FROM `ticket_master_entries` WHERE `status`!='Cancel' group by train_ticket_id)"));
$sq_train_pre_count = mysql_num_rows(mysql_query("select * from train_ticket_master where financial_year_id='$pre_financial_year_id' and train_ticket_id in(SELECT train_ticket_id FROM `ticket_master_entries` WHERE `status`!='Cancel' group by train_ticket_id)"));

$sq_train = mysql_fetch_assoc(mysql_query("select sum(net_total) as net_total from train_ticket_master where financial_year_id='$financial_year_id' and train_ticket_id in(SELECT train_ticket_id FROM `ticket_master_entries` WHERE `status`!='Cancel' group by train_ticket_id)"));
$sq_train_pre = mysql_fetch_assoc(mysql_query("select sum(net_total) as net_total from train_ticket_master where financial_year_id='$pre_financial_year_id' and train_ticket_id in(SELECT train_ticket_id FROM `ticket_master_entries` WHERE `status`!='Cancel' group by train_ticket_id)"));
...
$current_month_train = $sq_train_month['net_total'];
            ...
            <?php } 
 // <!-- Visa Booking -->
$sq_visa_count = mysql_num_rows(mysql_query("select * from visa_master where financial_year_id='$financial_year_id' and visa_id in(SELECT visa_id FROM `visa_master_entries` WHERE `status`!='Cancel' group by visa_id)"));
$sq_visa_pre_count = mysql_num_rows(mysql_query("select * from visa_master where financial_year_id='$pre_financial_year_id' and visa_id in(SELECT visa_id FROM `visa_master_entries` WHERE `status`!='Cancel' group by visa_id)"));

$sq_visa = mysql_fetch_assoc(mysql_query("select sum(visa_total_cost) as net_total from visa_master where financial_year_id='$financial_year_id' and visa_id in(SELECT visa_id FROM `visa_master_entries` WHERE `status`!='Cancel' group by visa_id)"));
$sq_visa_pre = mysql_fetch_assoc(mysql_query("select sum(visa_total_cost) as net_total from visa_master where financial_year_id='$pre_financial_year_id' and visa_id in(SELECT visa_id FROM `visa_master_entries` WHERE `status`!='Cancel' group by visa_id)"));
...
$current_month_visa = $sq_visa_month['net_total'];
            ...
            <?php }
// <!-- Car Rental Booking -->
$sq_car_count = mysql_num_rows(mysql_query("select * from car_rental_booking where financial_year_id='$financial_year_id' and status!='Cancel'"));
$sq_car_pre_count = mysql_num_rows(mysql_query("select * from car_rental_booking where financial_year_id='$pre_financial_year_id' and status!='Cancel'"));

$sq_car = mysql_fetch_assoc(mysql_query("select sum(total_fees) as net_total from car_rental_booking where financial_year_id='$financial_year_id' and status!='Cancel'"));
$sq_car_pre = mysql_fetch_assoc(mysql_query("select sum(total_fees) as net_total from car_rental_booking where financial_year_id='$pre_financial_year_id' and status!='Cancel'"));
...
$current_month_car = $sq_car_month['net_total'];
            ...
            <?php } 
// <!-- Activity Booking -->
$sq_exc_count = mysql_num_rows(mysql_query("select * from excursion_master where financial_year_id='$financial_year_id' and exc_id in(SELECT exc_id FROM `excursion_master_entries` WHERE `status`!='Cancel' group by exc_id)"));
$sq_exc_pre_count = mysql_num_rows(mysql_query("select * from excursion_master where financial_year_id='$pre_financial_year_id' and exc_id in(SELECT exc_id FROM `excursion_master_entries` WHERE `status`!='Cancel' group by exc_id)"));

$sq_exc = mysql_fetch_assoc(mysql_query("select sum(exc_total_cost) as net_total from excursion_master where financial_year_id='$financial_year_id' and exc_id in(SELECT exc_id FROM `excursion_master_entries` WHERE `status`!='Cancel' group by exc_id)"));
$sq_exc_pre = mysql_fetch_assoc(mysql_query("select sum(exc_total_cost) as net_total from excursion_master where financial_year_id='$pre_financial_year_id' and exc_id in(SELECT exc_id FROM `excursion_master_entries` WHERE `status`!='Cancel' group by exc_id)"));
...
$current_month_act = $sq_exc_month['net_total'];
            ...
            <?php } 
// <!-- misc Booking -->
$sq_misc_count = mysql_num_rows(mysql_query("select * from miscellaneous_master where financial_year_id='$financial_year_id' and misc_id in(SELECT misc_id FROM `miscellaneous_master_entries` WHERE `status`!='Cancel' group by misc_id)"));
$sq_misc_pre_count = mysql_num_rows(mysql_query("select * from miscellaneous_master where financial_year_id='$pre_financial_year_id' and misc_id in(SELECT misc_id FROM `miscellaneous_master_entries` WHERE `status`!='Cancel' group by misc_id)"));

$sq_misc = mysql_fetch_assoc(mysql_query("select sum(misc_total_cost) as net_total from miscellaneous_master where financial_year_id='$financial_year_id' and misc_id in(SELECT misc_id FROM `miscellaneous_master_entries` WHERE `status`!='Cancel' group by misc_id)"));
$sq_misc_pre = mysql_fetch_assoc(mysql_query("select sum(misc_total_cost) as net_total from miscellaneous_master where financial_year_id='$pre_financial_year_id' and misc_id in(SELECT misc_id FROM `miscellaneous_master_entries` WHERE `status`!='Cancel' group by misc_id)"));
...
$current_month_forex = $sq_forex_month['net_total'];
            ...
            <?php } 
// <!-- Passport Booking -->
$sq_passport_count = mysql_num_rows(mysql_query("select * from passport_master where financial_year_id='$financial_year_id' and passport_id in(SELECT passport_id FROM `passport_master_entries` WHERE `status`!='Cancel' group by passport_id)"));
$sq_passport_pre_count = mysql_num_rows(mysql_query("select * from passport_master where financial_year_id='$pre_financial_year_id' and passport_id in(SELECT passport_id FROM `passport_master_entries` WHERE `status`!='Cancel' group by passport_id)"));

$sq_passport = mysql_fetch_assoc(mysql_query("select sum(passport_total_cost) as net_total from passport_master where financial_year_id='$financial_year_id' and passport_id in(SELECT passport_id FROM `passport_master_entries` WHERE `status`!='Cancel' group by passport_id)"));
$sq_passport_pre = mysql_fetch_assoc(mysql_query("select sum(passport_total_cost) as net_total from passport_master where financial_year_id='$pre_financial_year_id' and passport_id in(SELECT passport_id FROM `passport_master_entries` WHERE `status`!='Cancel' group by passport_id)"));
