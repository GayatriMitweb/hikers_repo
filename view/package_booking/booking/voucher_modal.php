...
$sq_count_p = mysql_num_rows(mysql_query("select * from package_tour_transport_master where booking_id='$booking_id'"));
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
	  ...
	  <?php if($sq_count != 0 || $sq_count_p != 0){ ?>
		...
		<?php
		$sq_package = mysql_query("select * from package_tour_transport_master where booking_id='$booking_id'");
		while($row_entry1 = mysql_fetch_assoc($sq_package)){

			$count++;
			$q_transport = mysql_fetch_assoc(mysql_query("select * from b2b_transfer_master where entry_id='$row_entry1[transport_bus_id]'"));
			
			$sq_count_v = mysql_fetch_assoc(mysql_query("select * from package_tour_transport_voucher_entries where booking_id='$booking_id' and transport_bus_id='$row_entry1[transport_bus_id]'"));
			if($sq_count_v == 0){
				...
				$sq_entry_n = mysql_fetch_assoc(mysql_query("select * from package_tour_transport_voucher_entries where booking_id='$booking_id' and transport_bus_id='$row_entry1[transport_bus_id]'"));
				$driver_name = $sq_entry_n['driver_name'];
				$driver_contact = $sq_entry_n['driver_contact'];
				$confirm_by = $sq_entry_n['confirm_by'];
			}
			?>
					<div class="row">
					...
					<div class="row">
						<div class="col-md-4 col-sm-6 mg_bt_10">
							<input type="text" id="driver_name<?= $count?>" name="driver_name" placeholder="Driver Name"  onchange="validate_city(this.id)"  title="Driver Name" value="<?= $driver_name ?>">
						</div>
						<div class="col-md-4 col-sm-6 mg_bt_10">
							<input type="text" id="driver_contact<?= $count?>" name="driver_contact" placeholder="Driver Contact"  onchange="mobile_validate(this.id)"  title="Driver Contact" value="<?= $driver_contact ?>">
						</div>
						<div class="col-md-4 col-sm-6 mg_bt_10">
							<input type="text" id="confirm_by<?= $count?>" name="confirm_by<?= $count?>"  onchange="validate_city(this.id)"  placeholder="Confirmed by" title="Confirmed by" value="<?= $confirm_by ?>" />
						</div>
					</div>
		<?php } ?>
